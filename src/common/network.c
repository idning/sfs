...
static inline int sockaddrnumfill(struct sockaddr_in *sa, uint32_t ip,
                                  uint16_t port)
{
...
static inline int sockaddrfill(struct sockaddr_in *sa, const char *hostname,
...
static inline int sockresolve(const char *hostname, const char *service,
                              uint32_t * ip, uint16_t * port, int family,
                              int socktype, int passive)
{
...
int tcpresolve(const char *hostname, const char *service, uint32_t * ip,
               uint16_t * port, int passive)
{
    return sockresolve(hostname, service, ip, port, AF_INET, SOCK_STREAM,
                       passive);
}
...
{
    int yes = 1;
    return setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, (char *)&yes,
                      sizeof(int));
}
...
{
    int yes = 1;
    return setsockopt(sock, IPPROTO_TCP, TCP_NODELAY, (char *)&yes,
                      sizeof(int));
}
...
int tcpstrtoconnect(int sock, const char *hostname, const char *service,
                    uint32_t msecto)
{
    struct sockaddr_in sa;
...
int tcpstrlisten(int sock, const char *hostname, const char *service,
                 uint16_t queue)
{
    struct sockaddr_in sa;
    if (sockaddrfill(&sa, hostname, service, AF_INET, SOCK_STREAM, 1) < 0) {
...
int udpresolve(const char *hostname, const char *service, uint32_t * ip,
               uint16_t * port, int passive)
{
    return sockresolve(hostname, service, ip, port, AF_INET, SOCK_DGRAM,
                       passive);
}
...
int udpwrite(int sock, uint32_t ip, uint16_t port, const void *buff,
             uint16_t leng)
{
    struct sockaddr_in sa;
    if (leng > 512) {
    ...
    sockaddrnumfill(&sa, ip, port);
    return sendto(sock, buff, leng, 0, (struct sockaddr *)&sa,
